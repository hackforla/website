    ...
    function noResultsMessageComponent(filterParams, textColor) {
        let filterList = ``
        let hashmap = {"technologies" : "Technology", "languages" : "Language", "tools": "Tool", "programs" : "Program"}
        for (let key in filterParams) {
            if (filterParams[key].length) {
                for (let i = 0; i < filterParams[key].length; i++) {
                    filterList += `<p>"<span class='toolkit-red'>${hashmap[key]}: ${filterParams[key][i]}</span>"</p>`
                }
            }
        }
