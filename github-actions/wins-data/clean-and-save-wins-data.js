...
const WINS_API_KEY = process.env.WINS_API_KEY || 'AIzaSyDsY0jyeGRnVYmQco1Pt0fW3iDyfHiyYZE' ;
...
function create_container_object(keys,persons,unwanted_keys){
    ...
    persons.map(person=>{
        ...
        if(person[person.length -1] != 'FALSE'){
            ...
            unwanted_keys.length > 0 && unwanted_keys.forEach(key=>{delete person_obj[key]})
...
(async function main() {
    ...
    const write_ready_container = create_container_object(keys,persons,unwanted_keys);
