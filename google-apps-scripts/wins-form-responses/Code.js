...
function main() {
  ...
  const sortedKeyValueData = keyValueData.sort(
  ...
  const cleanedAndFormattedKeyValueData = JSON.stringify(
    sortedKeyValueData,
    null,
    5,
  );
  const encodedKeyValueData = Utilities.base64Encode(
    `${cleanedAndFormattedKeyValueData}`,
  ...
  if (!arrEq(sortedKeyValueData, keyValueContent)) {
    ...
    const writeResponse = ghrequests.updateWinsFile(
      keyValueFile,
      encodedKeyValueData,
      keyValueSha,
    );
...
function createIssue(e) {
  ...
  ghrequests.addIssueToProjectBoardColumn(
    issueResponse.body.id,
    COLUMN_KEYS["InReview"],
  );
}
...
function insertLatestFormSubmitIntoReviewSheet(formSubmitEvent) {
  ...
  const reviewSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Review");
  ...
  processedFormDataObject.set(
    "LinkedIn",
    rawFormDataObject["Linkedin URL (optional)"],
  );
  processedFormDataObject.set(
    "LinkedIn Picture Allowed ?",
    rawFormDataObject[
  ...
  processedFormDataObject.set(
    "GitHub",
    rawFormDataObject["Github URL (optional)"],
  );
  processedFormDataObject.set(
    "GitHub Picture Allowed ?",
    rawFormDataObject[
  ...
  processedFormDataObject.set(
    "Teams",
    rawFormDataObject["Select the team(s) you're on"],
  );
  processedFormDataObject.set(
    "Roles",
    rawFormDataObject["Select your role(s) on the team"],
  );
  processedFormDataObject.set(
    "Specific Roles",
    rawFormDataObject["What is/was your specific role? (optional)"],
  );
  processedFormDataObject.set(
    "Celebrations",
    rawFormDataObject["What do you want to celebrate (select all that apply)?"],
  );
  processedFormDataObject.set(
    "Overview",
    rawFormDataObject["Give us a brief overview"],
  );

  const memberSince =
    rawFormDataObject["When did you join Hack for LA? (optional)"];
  ...
  responsesSheet
    .getRange(formDataInsertedAt.rowStart, formDataInsertedAt.columnEnd + 1)
    .setFormula("=Review!A2");
  ...
  responsesSheet
    .getRange(formDataInsertedAt.rowStart, formDataInsertedAt.columnEnd + 2)
    .setFormula("=Review!B2");
}
...
function setDisplayHomepageFormulas() {
  const responsesSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Responses");
  const responsesValues = responsesSheet
    .getRange(2, 2, responsesSheet.getLastRow() - 1, 14)
    .getValues();
...
function compareResponsesAndReview() {
  ...
  const responsesValues = responsesSheet
    .getRange(2, 2, responsesSheet.getLastRow() - 1, numColumns)
    .getValues();

  //All cells from the DisplayReview column of the review sheet
  const reviewInfo = reviewSheet
    .getRange(2, 4, reviewSheet.getLastRow() - 1)
    .getValues();

  //All cells from the Display and Homepage columns of the review sheet
  const reviewDisplayHomepageValues = reviewSheet
    .getRange(2, 1, reviewSheet.getLastRow() - 1, 2)
    .getValues();
  ...
  if (reviewSheet.getLastRow() !== responsesSheet.getLastRow()) {
    console.log(
      "The Review and Response sheets have different lengths! \nThe review sheet has " +
        reviewSheet.getLastRow() +
        " rows \nThe responses sheet has " +
        responsesSheet.getLastRow() +
        " rows",
    );
    return;
  }
  ...
  if (unamatched === 0) {
    console.log(
      "SUCCESS! All elements from the review sheet match those from the response sheet",
    );
  } else {
    console.log(
